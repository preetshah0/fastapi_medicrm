...
branch_data: BranchCreate,
    ...
    if not db_org:
        error_response("Organization not found", data="")

    db_branch = db.query(Branch).filter(Branch.branch_email == branch_data.branch_email).first()
    if db_branch:
        error_response("Branch already exists", data="")
...
def get_branch_route(branch_id: str, db: Session = Depends(get_db)):
    ...
    if not db_branch:
        not_found_response("Branch not found")
...
def update_branch_route(
    ...
    if branch_data.branch_email:
        ...
        if db_branch_with_email:
            error_response("Branch already exists with this email", data="")
...
def assign_users_to_branches_route(
    ...
    for entry in payload.users:
        if not entry.user_id or not entry.role_id:
            not_found_response("Each user assignment must include user and role", data="")
        
        db_user = db.query(User).filter(User.id == entry.user_id).first()
        if not db_user:
            not_found_response(f"User not found: {entry.user_id}", data="")
            
        if db_user.organization_id != db_branch.organization_id:
            error_response(f"User {db_user.name} does not belong to the branch's organization", data="")
            
        db_role = db.query(Roles).filter(Roles.id == entry.role_id).first()
        if not db_role:
            not_found_response(f"Role not found: {db_role.name}", data="")
...
def users_branch_route(
    ...
    if not users:
        not_found_response("No users found in this branch", data=[])
